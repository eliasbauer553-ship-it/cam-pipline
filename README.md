...
pipeline/
  ...
  internet_archive.py          Erscheinungsdatum-Fallback (Fallback, kein Discovery-Schritt)
...
## Sechste Ergänzung: Internet Archive als Datum-Fallback

Nach demselben Muster wie Wikimedia Commons (siehe oben): für Kameras, die
nach Wikidata immer noch **kein Erscheinungsdatum** haben, wird eine
gezielte Suche gegen Internet Archives öffentliche Advanced-Search-API
versucht (`archive.org/advancedsearch.php`, kein API-Key nötig). Genutzt
wird ausschließlich das Metadaten-Feld `date` des frühesten plausiblen
Treffers (Jahr 1990–2030, sonst fast immer ein False Positive) — **nicht**
der archivierte Volltext/Inhalt selbst; dessen Urheberrecht bleibt
unabhängig davon beim jeweiligen Rechteinhaber.

Gleiche Entwurfsprinzipien wie beim Wikimedia-Fallback: kein eigener
Discovery-Schritt, hartes Limit pro Lauf (`INTERNET_ARCHIVE_LOOKUP_LIMIT`,
Default 300), bestehendes Wikidata-Datum wird nie überschrieben, Treffer
bekommen das Flag `datum_via_internet_archive_fallback`, deaktivierbar via
`--no-internet-archive`. Getestet mit gemocktem Netzwerk-Call in
`tests/test_internet_archive_fallback.py`.

**Von LensTip/OpticalLimits bewusst abgesehen:** Das wären keine offenen
Datensätze wie Wikidata/Internet Archive, sondern redaktionell erarbeitete
Messwerte kommerzieller Test-Websites — systematisches Scrapen davon würde
diese Pipeline nicht automatisch einbauen, unabhängig von kommerzieller
oder privater Nutzung des Ergebnisses. Für Objektiv-Testwerte (MTF-Charts
etc.) wären offiziell von Herstellern veröffentlichte PDF-Diagramme der
sauberere Weg, falls das später gewünscht ist.
