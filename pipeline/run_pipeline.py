...
def main(skip_fetch: bool = False, sources: list[str] | None = None,
         no_wikimedia: bool = False, no_internet_archive: bool = False) -> dict:
    active_sources = {
    ...
    enriched = run_enrichment(clusters, use_wikimedia_fallback=not no_wikimedia,
                               use_internet_archive_fallback=not no_internet_archive)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-internet-archive", action="store_true",
                         help="Internet-Archive-Datum-Fallback deaktivieren (z.B. für netzwerklose Testläufe)")
    args = parser.parse_args()
    main(skip_fetch=args.skip_fetch, sources=args.sources,
         no_wikimedia=args.no_wikimedia, no_internet_archive=args.no_internet_archive)
