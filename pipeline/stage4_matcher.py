...
def _similarity_multi(keys_a: list[str], keys_b: list[str]) -> tuple[float, str, str]:
    """Wie _similarity, aber über mehrere Namens-KANDIDATEN pro Seite
    (Hauptname + Aliase) — gibt den besten Score zurück, PLUS welches
    Kandidatenpaar ihn erzeugt hat (fürs Debugging/Diagnostics wichtig:
    man will sehen, ob der Hauptname oder ein Alias den Match gebracht hat)."""
    best_score, best_a, best_b = 0.0, keys_a[0], keys_b[0]
    for ka in keys_a:
        for kb in keys_b:
            s = _similarity(ka, kb)
            if s > best_score:
                best_score, best_a, best_b = s, ka, kb
    ...
    for (entity_type, brand), clusters in clusters_by_key.items():
        ...
        for i, a in enumerate(singles):
            for b in singles[i+1:]:
                ...
                score, matched_a, matched_b = _similarity_multi(a["model_keys"], b["model_keys"])
                if score >= threshold - 15 and score < threshold:  # "knapp daneben"-Fenster
                    near_misses.append({
                        "entity_type": entity_type, "brand": brand,
                        "a": matched_a, "a_source": a_src, "a_all_names": a["model_keys"],
                        "b": matched_b, "b_source": b_src, "b_all_names": b["model_keys"],
...
def run_matcher(normalized_records: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    Jeder Satz bringt nicht nur EINEN Namen mit, sondern eine Liste von
    Namens-Kandidaten (Hauptname + Aliase, z.B. Wikidatas "Sony Burano"
    plus Alias "ILME-FR7") — verglichen wird über ALLE Kombinationen,
    der beste Treffer zählt. Das ist der einzige generalisierbare Weg,
    Marketingname vs. internen Modellcode zu überbrücken, ohne eine von
    Hand gepflegte Alias-Tabelle zu brauchen.
    """
    ...
    for (entity_type, brand), recs in by_brand.items():
        clusters: list[dict] = []  # {"model_keys": [str, ...], "members": [rec, ...]}
        for rec in recs:
            primary = rec["model_normalized"] or rec["canonical_name_candidate"]
            aliases = rec.get("model_aliases_normalized") or []
            model_keys = [primary] + [a for a in aliases if a and a != primary]

            best_cluster, best_score = None, 0.0
            for cluster in clusters:
                score, _, _ = _similarity_multi(model_keys, cluster["model_keys"])
                if score > best_score:
            ...
            if best_cluster is not None and best_score >= config.FUZZY_MATCH_THRESHOLD:
                ...
                # Neue Namens-Kandidaten des beigetretenen Mitglieds stehen
                # künftigen Vergleichen ebenfalls zur Verfügung (dedupliziert).
                for k in model_keys:
                    if k not in best_cluster["model_keys"]:
                        best_cluster["model_keys"].append(k)
            else:
                group_counter += 1
                clusters.append({
