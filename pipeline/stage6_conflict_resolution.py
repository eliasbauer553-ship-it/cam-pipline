...
def resolve_field(field: str, candidates: list[dict]) -> tuple[object, list[dict] | None, str]:
    """Gibt (gewaehlter_wert, konfliktliste_oder_None, gewinnende_quelle) zurueck."""
    distinct_values = {c["value"] for c in candidates}
    if len(distinct_values) == 1:
        return candidates[0]["value"], None, candidates[0]["source"]

    priority = _priority_for(field)
    for preferred_source in priority:
        for c in candidates:
            if c["source"] == preferred_source:
                return c["value"], candidates, c["source"]  # Konflikt vorhanden, aber aufgelöst

    # Keine der priorisierten Quellen war vertreten -> nimm den ersten Kandidaten,
    # aber markiere es deutlich als Notlösung.
    return candidates[0]["value"], candidates, candidates[0]["source"]
...
def resolve_record(enriched: dict) -> dict:
    ...
    for field, candidates in enriched["field_candidates"].items():
        value, conflict_list, winning_source = resolve_field(field, candidates)
        resolved_fields[field] = value
        field_sources[field] = winning_source
    ...
    return {
        ...
        **resolved_fields,
        "_field_sources": field_sources,   # welches Feld kam von welcher Quelle
