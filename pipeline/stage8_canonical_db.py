...
def build_canonical_camera(rec: dict) -> dict:
    return {
        ...
        "image_license_hint": rec.get("image_license_hint"),
...
def run_canonical_db(scored_records: list[dict], quality_report: dict, match_diagnostics: list[dict] | None = None) -> dict:
    ...
    if match_diagnostics:
        diag_path = config.CANONICAL_DIR / "match_diagnostics.json"
        diag_path.write_text(json.dumps({
            "generated_at": timestamp,
            "explanation": (
                "Quellenübergreifende Modellnamen-Paare, deren Ähnlichkeit knapp unter der "
                f"Match-Schwelle ({config.FUZZY_MATCH_THRESHOLD}) lag. Jeder Eintrag ist ein "
                "Kandidat, bei dem zwei Quellen wahrscheinlich dieselbe Kamera/dasselbe Objektiv "
                "meinen, aber als getrennte Einträge gelandet sind. Zum Nachschärfen von "
                "config.FUZZY_MATCH_THRESHOLD oder stage3_normalizer.MODEL_CLEAN_RULES nutzen."
            ),
            "count": len(match_diagnostics),
            "pairs": match_diagnostics,
        }, ensure_ascii=False, indent=2))
        log.info("Match-Diagnose geschrieben: %s (%s Paare)", diag_path, len(match_diagnostics))
