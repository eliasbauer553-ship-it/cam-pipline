...
from pipeline.stage4_matcher import run_matcher, _similarity, _similarity_multi, _diagnose_near_misses  # noqa: E402
...
def check_3_diagnostic_mechanism_fires():
    ...
    fake_clusters = [
        {"model_keys": ["Testmodell Alpha"], "members": [{"_source": "wikidata"}]},
        {"model_keys": ["Testmodell Beta"],  "members": [{"_source": "camera_sensor_db"}]},
    ]
    score, _, _ = _similarity_multi(fake_clusters[0]["model_keys"], fake_clusters[1]["model_keys"])
...
def check_4_alias_bridges_vocabulary_gap():
    """Der eigentliche Auslöser dieser Erweiterung: Wikidata nennt eine
    Kamera beim Marketingnamen ('Burano'), hat aber 'ILME-FR7' als Alias
    hinterlegt. camera_sensor_db kennt nur den internen Modellcode. Ohne
    Alias-Unterstützung: 0% Ähnlichkeit, kein Match. Mit Alias: Match."""
    raw = [
        {"_source": "wikidata", "label_raw": "Sony Burano", "manufacturer_raw": "Sony",
         "aliases_raw": ["ILME-FR7", "Sony ILME-FR7"],
         "image_url": "https://example.org/burano.jpg", "release_date": "2023-11-08", "mount_raw": "Sony E"},
        {"_source": "camera_sensor_db", "vendor_raw": "Sony", "model_raw": "ILME-FR7", "sensor_modes": {}},
    ]
    normalized = [normalize_record(r) for r in raw]
    clusters, _ = run_matcher(normalized)
    assert len(clusters) == 1, (
        f"FEHLGESCHLAGEN: 'Sony Burano' (Alias 'ILME-FR7') und 'ILME-FR7' wurden NICHT "
        f"zusammengeführt — bekam {len(clusters)} Cluster statt 1."
    )
    print(f"OK (4/4): 'Sony Burano' (Wikidata, Alias 'ILME-FR7') + 'ILME-FR7' (camera_sensor_db) "
          f"-> 1 Cluster über den Alias gefunden (Confidence {clusters[0]['match_confidence']}).")
...
def main():
    ...
    check_4_alias_bridges_vocabulary_gap()
